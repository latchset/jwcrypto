...
class TestOverloadedOperators(unittest.TestCase):
    ...
    def test_jwt_equality(self):
        key = jwk.JWK.generate(kty="oct", size=256)
        a = jwt.JWT(header={"alg": "HS256"}, claims={"info": "I'm a signed token"})
        b = jwt.JWT(header={"alg": "HS256"}, claims={"info": "I'm a signed token"})
        self.assertEqual(a, b)
        ...
        ea = jwt.JWT(
            header={"alg": "A256KW", "enc": "A256CBC-HS512"}, claims=a.serialize()
        )
        eb = jwt.JWT(
            header={"alg": "A256KW", "enc": "A256CBC-HS512"}, claims=b.serialize()
        )
